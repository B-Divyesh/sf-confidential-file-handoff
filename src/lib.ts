...
export function recipientSheet(details: SheetDetails): string {
  ...
  return `CONFIDENTIAL FILE HANDOFF\n\nHello ${recipient},\n\nYour sender prepared ${details.fileCount} ${fileWord} in a protected AES-256 ZIP.\n\n1. Receive or download the file named confidential-file-handoff.zip using: ${details.delivery}.\n2. Save confidential-file-handoff.zip somewhere you can find again, such as Downloads.\n3. Open confidential-file-handoff.zip. It will ask for the ZIP access phrase.\n4. The ZIP access phrase will arrive separately by ${details.passwordChannel}. Do not expect it in the same message as the ZIP.\n5. After opening the files, reply to your sender: “I opened the files.”\n${note}\nIf your computer's built-in ZIP tool says the archive is unsupported, use an AES-256 ZIP-compatible extractor such as 7-Zip (Windows), Keka (macOS), or PeaZip (Windows, macOS, or Linux). If it still does not open, tell your sender what device and app you are using. Do not send the ZIP access phrase back in the same message as the ZIP.\n\nWhat this does: the protected ZIP needs its separate ZIP access phrase before its contents can be opened.\nWhat this does not do: hide file names shown by a ZIP listing, verify who is receiving it, protect a device after it is opened, or guarantee the safety of the delivery channel.\n`;
}
