...
import './style.css';
import { HandoffStore, makePassword, recipientSheet, uniqueArchiveNames, type HandoffRecord } from './lib';
...
form.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    for (const [index, file] of files.entries()) await writer.add(entryNames[index], new BlobReader(file), { password: passwordInput.value, encryptionStrength: 3 });
    ...
    try {
      ...
      setStatus('Encrypted ZIP created. Your browser could not save the optional local checklist, but both downloads are ready below.', 'busy');
    }
...
document.querySelector('#print-sheet')!.addEventListener('click', () => {
  ...
  if (!popup) { setStatus('The print window was blocked. Allow pop-ups for this site, then choose Print the sheet again.', 'error'); return; }
  popup.opener = null;
  const safeSheet = preparedSheet.replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;');
  popup.document.write(`<!doctype html><html lang="en"><head><title>Recipient handoff sheet</title><meta name="viewport" content="width=device-width"><style>body{margin:0;padding:2rem;color:#111;background:#fff}pre{white-space:pre-wrap;font:16px/1.5 system-ui,sans-serif;max-width:48rem;margin:auto}@media print{body{padding:0}}</style></head><body><main><pre>${safeSheet}</pre></main></body></html>`);
...
async function updateActive(field: 'sentAt' | 'acknowledgedAt', checked: boolean) { if (!activeRecord) return; activeRecord[field] = checked ? new Date().toISOString() : undefined; try { await store.put(activeRecord); await renderRecords(); } catch { setStatus('That checklist change could not be saved in this browser.', 'error'); } }
...
async function renderRecords() {
  ...
  try {
    ...
    records.forEach((record) => {
      ...
      ([['sentAt', 'Mark ZIP and sheet sent', 'Sent'], ['acknowledgedAt', 'Mark files opened', 'Opened']] as const).forEach(([field, labelText, doneText]) => {
        ...
        text.textContent = record[field] ? `${doneText} ${displayDate(record[field])}` : labelText;
        checkbox.addEventListener('change', async () => {
          checkbox.disabled = true;
          record[field] = checkbox.checked ? new Date().toISOString() : undefined;
          try {
            ...
            checkbox.disabled = false;
            setStatus('That checklist change could not be saved in this browser.', 'error');
          }
        });
      ...
      remove.addEventListener('click', async () => { if (confirm(`Delete the local handoff log for ${record.recipient}? This cannot be undone.`)) { await store.remove(record.id); await renderRecords(); } });
...
if ('serviceWorker' in navigator) {
  ...
  navigator.serviceWorker.addEventListener('controllerchange', () => {
    if (hadController) document.querySelector<HTMLElement>('#update-toast')!.hidden = false;
    hadController = true;
  });
